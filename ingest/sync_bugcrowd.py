...
Sync Bugcrowd programs/scopes into the normalized schema.

Requirements:
- DB_DSN
- BUGCROWD_API_TOKEN (or BUGCROWD_AUTH_HEADER)
- BUGCROWD_PROGRAMS_URL (list endpoint)
- optional BUGCROWD_SCOPES_URL_TEMPLATE (format with {program_id})

Notes:
- Idempotent upserts into programs/scopes (platform='bugcrowd')
...
PLATFORM = "bugcrowd"
PROGRAMS_URL = os.getenv("BUGCROWD_PROGRAMS_URL", "").strip()
SCOPES_URL_TEMPLATE = os.getenv("BUGCROWD_SCOPES_URL_TEMPLATE", "").strip()
TOKEN = os.getenv("BUGCROWD_API_TOKEN", "").strip()
AUTH_HEADER = os.getenv("BUGCROWD_AUTH_HEADER", "").strip()
TOKEN_TYPE = os.getenv("BUGCROWD_TOKEN_TYPE", "Token").strip() or "Token"

HTTP_TIMEOUT = int(os.getenv("BUGCROWD_HTTP_TIMEOUT", "60"))
HTTP_RETRIES = int(os.getenv("BUGCROWD_HTTP_RETRIES", "3"))
HTTP_RETRY_SLEEP = float(os.getenv("BUGCROWD_HTTP_RETRY_SLEEP", "2"))
...
def clean_identifier(asset_type: str, identifier: str) -> str:
    ...
    if asset_type == "url":
        # keep URL but normalize scheme/lower host
        try:
            p = urlparse(ident)
...
def normalize_asset_type_bc(raw_type: Optional[str], identifier: str) -> str:
    at = (raw_type or "").strip().lower()
    if at in {"mobile_app", "downloadable"}:
        return at
    # fallback to common normalizer
    return normalize_asset_type(PLATFORM, raw_type, identifier)


def main():
    if not TOKEN and not AUTH_HEADER:
        raise RuntimeError("Missing BUGCROWD_API_TOKEN (or BUGCROWD_AUTH_HEADER) in .env")
    if not PROGRAMS_URL:
        raise RuntimeError("Missing BUGCROWD_PROGRAMS_URL in .env")
    ...
    with psycopg.connect(DB_DSN) as conn:
        ...
        for item in programs:
            ...
            offers_bounties = item.get("offers_bounties")
            if offers_bounties is None and "bounty" in item:
                offers_bounties = item.get("bounty")
            currency = item.get("currency") or item.get("bounty_currency")
            policy = item.get("policy") or item.get("url") or item.get("website")

            upsert_program(conn, PLATFORM, external_id, handle, name, offers_bounties, currency, policy, item)
            programs_upserted += 1
            ...
            if scopes:
                with conn.cursor() as cur:
                    for sc in scopes:
                        ...
                        if not identifier:
                            continue
                        asset_type = normalize_asset_type_bc(raw_type, identifier)
            ...
            # add fingerprints from scopes and program
