...
Sync Intigriti programs/scopes into the normalized schema.

Requirements:
- DB_DSN
- INTIGRITI_PROGRAMS_URL (list endpoint)
- optional INTIGRITI_SCOPES_URL_TEMPLATE (format with {program_id})
- optional INTIGRITI_API_TOKEN / INTIGRITI_AUTH_HEADER

Notes:
- Idempotent upserts into programs/scopes (platform='intigriti')
...
PLATFORM = "intigriti"
PROGRAMS_URL = os.getenv("INTIGRITI_PROGRAMS_URL", "").strip()
SCOPES_URL_TEMPLATE = os.getenv("INTIGRITI_SCOPES_URL_TEMPLATE", "").strip()
TOKEN = os.getenv("INTIGRITI_API_TOKEN", "").strip()
AUTH_HEADER = os.getenv("INTIGRITI_AUTH_HEADER", "").strip()
TOKEN_TYPE = os.getenv("INTIGRITI_TOKEN_TYPE", "Bearer").strip() or "Bearer"

HTTP_TIMEOUT = int(os.getenv("INTIGRITI_HTTP_TIMEOUT", "60"))
HTTP_RETRIES = int(os.getenv("INTIGRITI_HTTP_RETRIES", "3"))
HTTP_RETRY_SLEEP = float(os.getenv("INTIGRITI_HTTP_RETRY_SLEEP", "2"))
...
def request_json(session: requests.Session, url: str, params=None) -> Optional[dict]:
    ...
    for attempt in range(1, HTTP_RETRIES + 1):
        try:
            ...
            last_txt = r.text or ""
            if st in (401, 403):
                log("[FATAL] Intigriti API requires a token. Set INTIGRITI_API_TOKEN in .env.")
                return None
...
def main():
    if not PROGRAMS_URL:
        raise RuntimeError("Missing INTIGRITI_PROGRAMS_URL in .env")
